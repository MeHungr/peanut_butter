...
func (s *Storage) GetTargets(filter AgentFilter) ([]Agent, error) {
	...
	if !filter.All {
		// If filtering by id
		if len(filter.IDs) > 0 {
			clause, idArgs, _ := sqlx.In("agent_id IN (?)", filter.IDs)
...
func (s *Storage) AddTargets(filter AgentFilter) error {
	base := `UPDATE agents SET targeted = 1`
	var whereClause []string
	var args []any

	// If no 'All' flag and no filters, do nothing
    if !filter.All && len(filter.IDs) == 0 && len(filter.OSes) == 0 && len(filter.Statuses) == 0 {
        return fmt.Errorf("no filter provided and 'all' flag not set")
    }
...
func (s *Storage) Untarget(filter AgentFilter) error {
	base := `UPDATE agents SET targeted = 0`
	var whereClause []string
	var args []any
...
func (s *Storage) SetTargets(filter AgentFilter) error {
	// Clear targets
	...
	if err := s.AddTargets(filter); err != nil {
		return fmt.Errorf("SetTargets: %w", err)
	}
